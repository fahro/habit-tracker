...
export default function HomePage({ users, selectedUserId, onUserChange }) {
  ...
  return (
    <div className="h-full flex flex-col md:flex-row bg-background">
      ...
      <div className="md:hidden bg-white border-b border-slate-100 sticky top-0 z-10">
        <div className="max-w-lg mx-auto px-4 pt-3 pb-0">
          <div className="flex items-center justify-between mb-2">
            ...
            <button onClick={goAdmin} className="text-slate-300 hover:text-slate-500 transition-colors p-1 -mr-1">
      ...
      <div className="hidden md:flex flex-col bg-white border-r border-slate-100 w-60 flex-shrink-0 h-full">
        <div className="flex flex-col h-full p-5">
          {/* App title */}
          <div className="mb-6">
            <h1 className="text-lg font-bold text-slate-900 tracking-tight">Habit Tracker</h1>
            <p className="text-xs text-slate-400 mt-0.5">{dateLabel}</p>
          </div>

          {/* User selector */}
          <div className="mb-6">
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-2">User</p>
              ...
              {users.map(u => {
                ...
                return (
                  <button
                    ...
                    style={active ? { background: '#6366f120', color: '#6366f1' } : { color: '#64748b' }}
                  >
                    <span className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0"
                      style={active ? { background: '#6366f1', color: 'white' } : { background: '#e2e8f0', color: '#64748b' }}>
                      {name[0].toUpperCase()}
                    </span>
                    {name}
                  </button>
          ...
          <nav className="flex flex-col gap-1 flex-1">
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-2">Menu</p>
            {TABS.map(({ id, label, Icon }) => {
              const active = tab === id
              return (
                <button
                ...
                >
                  <Icon className="w-4.5 h-4.5 flex-shrink-0" strokeWidth={active ? 2.5 : 2} />
                  {label}
                </button>
              )
            })}
          ...
          <button
            onClick={goAdmin}
            className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm font-semibold text-slate-400 hover:bg-slate-50 hover:text-slate-600 transition-all"
          >
      ...
      <div className="flex-1 overflow-y-auto" style={{ WebkitOverflowScrolling: 'touch', overscrollBehavior: 'contain' }}>
        <div className="max-w-lg mx-auto md:max-w-none px-4 md:px-8 py-5 md:py-8"
                                ...
                                {habitLogs.map(log => (
                                  <div key={log.id} className="flex items-center gap-2">
                                    ...
                                    <button
                                      ...
                                      className="text-slate-300 hover:text-danger transition-colors p-1.5 flex-shrink-0 disabled:opacity-40"
                                    >
